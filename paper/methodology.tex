% Methodology Section for CTCA/CTAA Paper
% Compile with: pdflatex methodology.tex

\section{Methodology}
\label{sec:methodology}

We present \textbf{Cross-Timestep Amortization} (CTA), a suite of techniques that exploit temporal coherence in diffusion models to accelerate sparse attention computation. Our approach consists of two complementary components: \textbf{Cross-Timestep Cluster Amortization} (CTCA) for reducing K-means clustering overhead, and \textbf{Cross-Timestep Attention Amortization} (CTAA) for hierarchical sparse attention computation. Figure~\ref{fig:overview} illustrates the overall architecture.
...
SAP~\cite{sap} addresses this by clustering tokens using K-means and permuting them to achieve block-diagonal sparsity:
\begin{equation}
...
\subsection{Cross-Timestep Cluster Amortization (CTCA)}
\label{sec:ctca}

CTCA reduces K-means overhead by intelligently reusing cluster assignments across timesteps. Instead of performing full clustering at every step, we maintain a cluster cache and selectively update it based on quality metrics.
...
The complete CTCA algorithm is presented in Algorithm~\ref{alg:ctca}.

\begin{algorithm}[t]
\caption{Cross-Timestep Cluster Amortization (CTCA)}
\label{alg:ctca}
\begin{algorithmic}[1]
...
\subsection{Cross-Timestep Attention Amortization (CTAA)}
\label{sec:ctaa}

While CTCA accelerates clustering, the dominant cost in sparse attention is still the attention computation itself. CTAA introduces a hierarchical attention mechanism that further exploits temporal coherence in attention patterns.

\paragraph{Hierarchical Attention Tiers.}
We partition cluster pairs into three tiers based on their attention importance (Figure~\ref{fig:ctaa_tiers}):
...
The complete CTAA procedure is detailed in Algorithm~\ref{alg:ctaa}.

\begin{algorithm}[t]
\caption{Cross-Timestep Attention Amortization (CTAA)}
\label{alg:ctaa}
\begin{algorithmic}[1]
...
where $R$ is the CTCA reuse ratio (typically $\sim$7$\times$), $B$ is the average block size, and $p_{\text{full}}$ is the full attention threshold (typically 0.7).

\paragraph{Theoretical Speedup.}
The combined speedup from CTCA and CTAA can be estimated as:
\begin{equation}
    S_{\text{total}} = \frac{1}{p_{\text{full}} + p_{\text{centroid}} \cdot \epsilon + p_{\text{skip}} \cdot 0}
\end{equation}
where $\epsilon \ll 1$ is the relative cost of centroid attention. With typical parameters ($p_{\text{full}}=0.7$, $p_{\text{total}}=0.95$), this yields $S_{\text{total}} \approx 1.4\times$ on attention computation, combined with $7\times$ K-means speedup from CTCA.

\subsection{Memory-Efficient Implementation}
\label{sec:memory}

To enable video generation on consumer GPUs (24GB), we implement dynamic layer offloading:

\paragraph{Layer Windowing.}
We maintain a sliding window of $W$ transformer layers on GPU (typically $W=8$), with remaining layers on CPU. As inference progresses, we prefetch the next layer asynchronously:
\begin{equation}
    \text{GPU}_t = \{\ell : t \leq \ell < t + W\}, \quad \text{Prefetch: } \ell_{t+W}
\end{equation}

\paragraph{CPU Cluster Cache.}
CTCA caches are stored on CPU memory to minimize GPU memory pressure. The cache size scales as $\mathcal{O}(L \cdot C \cdot d)$ where $L$ is the number of layers, resulting in approximately 3GB of CPU memory usage.

\subsection{Implementation Details}
\label{sec:implementation}

We implement CTCA and CTAA as drop-in replacements for standard attention in the HunyuanVideo~\cite{hunyuanvideo} pipeline using PyTorch and Triton. Key implementation choices:

\begin{itemize}
    \item \textbf{CTCA Parameters}: Quality threshold $\tau_q = 0.8$, max interval $\Delta_{\max} = 10$ timesteps, adaptive interval based on quality trends.
    \item \textbf{CTAA Parameters}: $p_{\text{full}} = 0.7$, $p_{\text{total}} = 0.95$, configurable for quality-speed tradeoff.
    \item \textbf{Warmup}: First 7.5\% of timesteps and 2.5\% of layers use full precision attention to ensure generation quality.
    \item \textbf{Hardware}: Tested on NVIDIA RTX 4090 (24GB) with dynamic offloading enabled.
\end{itemize}
